...
using MediatR;
using HR.Analytics.Application.Mappings;

/// <summary>
/// Handler for GetDashboardMetricsQuery.
/// SOLID: Handler separated from Query definition.
/// Uses centralized mapping (DashboardDtoMappingConfiguration) - NO duplicate mapping logic.
/// </summary>
public class GetDashboardMetricsQueryHandler : IRequestHandler<GetDashboardMetricsQuery, DashboardMetricsDto>
{
    ...
    public async Task<DashboardMetricsDto> Handle(GetDashboardMetricsQuery request, CancellationToken cancellationToken)
    {
        ...
        // Use centralized mapping (SOLID - no duplicate mapping logic)
