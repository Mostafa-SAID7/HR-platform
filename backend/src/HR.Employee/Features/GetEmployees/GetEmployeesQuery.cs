...
/// </summary>
public record GetEmployeesQuery(EmployeeFilterDto Filter, Guid TenantId) : IQuery<PaginatedResult<EmployeeListDto>>;
...
public class GetEmployeesQueryHandler : IRequestHandler<GetEmployeesQuery, PaginatedResult<EmployeeListDto>>
{
    ...
    public async Task<PaginatedResult<EmployeeListDto>> Handle(GetEmployeesQuery request, CancellationToken cancellationToken)
    {
        ...
        var items = (await _queryRepository.QueryAsync<EmployeeListDto>(sql, 
        ...
        return PaginatedResult<EmployeeListDto>.Create(items, filter.PageNumber, filter.PageSize, totalCount);
    }
}
